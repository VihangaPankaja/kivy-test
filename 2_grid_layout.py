...
class grid(GridLayout):
	def __init__(self, **kwargs):
		...
		self.add_widget(Label(text = "text field 1"))

		self.text1 = TextInput(multiline = False)
		self.add_widget(self.text1)

		self.add_widget(Label(text = "text field 2"))

		self.text2 = TextInput(multiline = False)
		self.add_widget(self.text2)

		self.btn = Button(text = "play", size_hint = (2,1), background_normal = '', background_color = (136/255, 176/255, 75/255, 1), color = (1,0,0,1), border = (0,0,0,0))
		self.btn.bind(on_press = self.press, on_release = self.release)
